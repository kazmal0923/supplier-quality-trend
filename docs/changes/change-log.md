...
| 2026-08-05 | 未Commit | FEATURE-002 | 仕入先チェックボックス複数選択と国内／海外区分合算を追加。合算は生成済み月別数量のブラウザ合算。既存グループモードは維持 | FEATURE-002 |
| 2026-08-05 | 未Commit | グループ構成明示 | 仕入先グループ候補に構成仕入先IDを明示。`supplierNames`を`supplierIds`順に揃える | FEATURE-001 |
